...
export default function Tabs() {
  ...
  const handleToggle = (e: { stopPropagation: () => void }) => {
    e.stopPropagation();
    isTabsSHow((prev) => !prev);
    console.log("Clicked");

    hiddenTabs();
  };
  const showNothing = (e: { stopPropagation: () => void }) => {
    e.stopPropagation();
    isTabsSHow((prev) => !prev);
    console.log("Clicked");
    showTabs();
    ...
    <div className="w-full relative flex">
      {isVisible && (
        <div
          ...
          className=" Hide px-4 py-2 z-10000 border border-gray-300 ml-2 bg-[#f5f5f5] w-max rounded-lg self-end flex items-center gap-3 text-base font-semibold md:flex cursor-pointer"
        >
          <svg
      ...
      {tabsShow && (
        <div className="z-700 pb-4 flex flex-col bg-[#f5f5f5] md:flex w-full h-max md:w-full md:h-max gap-4 p-[16px] md:pb-0 md:p-1 items-center justify-center">
          <div className="buttons border border-[1px] w-full rounded-lg p-1 border-gray-300 bg-[#f5f5f5] flex flex-col md:flex-row gap-3">
            {tabs.map((tabs) => (
              <Link key={tabs.name} href={tabs.href} className="">
                <button
                  className={clsx(
                    "cursor-pointer w-full md:hover:bg-[#fcfcfc] hover:rounded-lg hover:shadow-lg pl-2 py-2 gap-5 self-center md:w-[160px] md:h-[35px] h-[35px] text-[8px] text-[#171821] text-base md:text-[13px]",
                    pathname === tabs.href
